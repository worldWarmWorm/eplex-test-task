...
abstract class TestCase extends Assert implements Reorderable, SelfDescribing, Test
{
    ...
    private function stopOutputBuffering(): bool
    {
        ...
        if ($bufferingLevel !== $this->outputBufferingLevel) {
            if ($bufferingLevel > $this->outputBufferingLevel) {
                $message = 'tests code or tested code did not close its own output buffers';
            } else {
                $message = 'tests code or tested code closed output buffers other than its own';
            }
    ...
    private function restoreGlobalErrorExceptionHandlers(): void
    {
        ...
        if ($activeErrorHandlers !== $this->backupGlobalErrorHandlers) {
            if (count($activeErrorHandlers) > count($this->backupGlobalErrorHandlers)) {
                if (!$this->inIsolation) {
                    $message = 'tests code or tested code did not remove its own error handlers';
                }
            } else {
                $message = 'tests code or tested code removed error handlers other than its own';
            }
        ...
        if ($activeExceptionHandlers !== $this->backupGlobalExceptionHandlers) {
            if (count($activeExceptionHandlers) > count($this->backupGlobalExceptionHandlers)) {
                if (!$this->inIsolation) {
                    $message = 'tests code or tested code did not remove its own exception handlers';
                }
            } else {
                $message = 'tests code or tested code removed exception handlers other than its own';
            }
